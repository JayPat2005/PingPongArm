...
def track_tennis_ball_top():
    ...
    while True:
        ...
        if contours:
            max_contour = max(contours, key=cv2.contourArea)
            ((x, z), radius) = cv2.minEnclosingCircle(max_contour)  # Change (x, y) to (x, z)
            center = (int(x), int(z))  # Change (x, y) to (x, z)
            cv2.circle(frame, center, int(radius), (0, 255, 0), 2)  
            ball_position = center
            tail.append((center, time.time()))
            ...
            # Update the histogram for adaptive color range
            roi = hsv[int(z - radius):int(z + radius), int(x - radius):int(x + radius)]  # Change (y - radius) to (z - radius)
            hist = cv2.calcHist([roi], [0, 1], None, [180, 256], [0, 180, 0, 256])
            cv2.normalize(hist, hist, 0, 255, cv2.NORM_MINMAX)
            dst = cv2.calcBackProject([hsv], [0, 1], hist, [0, 180, 0, 256], 1)

           
            track_window = (int(x - radius), int(z - radius), max(1, int(2 * radius)), max(1, int(2 * radius)))  # Change (x - radius, y - radius) to (x - radius, z - radius)
            ret, track_window = cv2.meanShift(dst, track_window, criteria)
            x, z, w, h = track_window  # Change (x, y, w, h) to (x, z, w, h)
            center = (int(x + w / 2), int(z + h / 2))  # Change (x + w / 2, y + h / 2) to (x + w / 2, z + h / 2)

            # Calculate velocity
            if len(tail) > 1:
                dt = tail[-1][1] - tail[-2][1]  # Time difference
                dx = tail[-1][0][0] - tail[-2][0][0]  # Change in x
                dz = tail[-1][0][1] - tail[-2][0][1]  # Change in z
                velocity_x = dx / dt
                velocity_z = dz / dt

                cv2.putText(frame, f"X: {tail[-1][0][0]}, Z: {tail[-1][0][1]}", (10, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                cv2.putText(frame, f"VX: {velocity_x:.2f}, VZ: {velocity_z:.2f}", (10, 60),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
